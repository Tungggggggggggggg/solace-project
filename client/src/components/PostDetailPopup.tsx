...
const PostDetailPopup = ({ post, onClose, theme = 'inspiring' }: PostDetailPopupProps) => {
  ...
  useLayoutEffect(() => {
    ...
    if (imgRef.current) {
      gsap.fromTo(imgRef.current, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: 0.5, delay: 0.2, ease: "power2.out" });
    }
    if (contentRef.current) {
      gsap.fromTo(contentRef.current, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.5, delay: 0.3, ease: "power2.out" });
    }
    if (commentsRef.current) {
      gsap.fromTo(commentsRef.current, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.5, delay: 0.4, ease: "power2.out" });
    }
    return () => {
      if (popupRef.current) {
        gsap.to(popupRef.current, { scale: 0.9, opacity: 0, y: 50, duration: 0.3, ease: "power3.in" });
      }
    };
  }, []);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl max-w-2xl w-full shadow-2xl z-10 flex flex-col" style={{ maxHeight: '90vh' }}>
        {/* Tiêu đề cố định trên cùng modal, không scroll */}
        <div style={{ position: 'relative', zIndex: 30, background: 'rgba(255,255,255,0.97)', backdropFilter: 'blur(2px)', borderBottom: '1px solid #eee', borderTopLeftRadius: '1rem', borderTopRightRadius: '1rem', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>
          <h2 className="text-center font-bold text-xl text-gray-900 tracking-tight w-full">Bài viết của {post.name}</h2>
          <button
        ...
        {/* Nội dung scrollable */}
        <div ref={popupRef} className="flex-1 overflow-y-auto p-6" style={{ maxHeight: 'calc(90vh - 64px)' }}>
          {/* Header */}
          <div className="flex items-center mb-6">
            ...
            <div>
              <h3 className="text-gray-900 font-semibold text-lg">{post.name}</h3>
                ...
                <div className="flex gap-3 mt-3 overflow-x-auto pb-2">
                  {post.images.map((img, idx) => (
                    <img
                      ...
                      src={img}
                      alt={`thumb-${idx}`}
                      className={`w-20 h-20 object-cover rounded-lg border-2 transition-all ${selectedImg === idx ? "border-blue-500" : "border-transparent hover:border-blue-300"}`}
          ...
          {post.shared_post && (
            <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 mb-6 shadow-sm">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <span className="font-semibold text-gray-800">{post.shared_post.name}</span>
                  <span className="text-gray-500 text-xs ml-2">{formatDate(post.shared_post.date)}</span>
                </div>
              </div>
              <div
              ...
              >
                {post.shared_post.content && post.shared_post.content.length > 300 && !showFullSharedContent ? (
                  <>
              ...
              {post.shared_post.images && post.shared_post.images.length > 0 && (
                <div className={`grid ${post.shared_post.images.length > 1 ? "grid-cols-2" : "grid-cols-1"} gap-3 mt-3`}>
                  {post.shared_post.images.map((img, idx) => (
                    <Image key={idx} src={img} alt={`post-img-${idx}`} width={500} height={300} className="object-cover w-full h-48 rounded-lg transition-transform hover:scale-105" />
                  ))}
                </div>
              )}
            </div>
          ...
          <div className="mb-6" ref={contentRef}>
            ...
            >
              {post.content && post.content.length > 300 && !showFullContent ? (
                <>
